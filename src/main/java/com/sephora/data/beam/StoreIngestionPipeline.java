...
public class StoreIngestionPipeline {

    public interface StorePipelineOptions extends PipelineOptions {
    ...
    public static void main(String[] args) {
        StorePipelineOptions options =
                PipelineOptionsFactory.fromArgs(args).withValidation().as(StorePipelineOptions.class);
        ...
        if ("full".equalsIgnoreCase(options.getMode())) {
            rows = pipeline
                    ...
                    .apply("ParseFullCsv", ParDo.of(new ParseStoreFullCsvDoFn()));
            ...
            rows = pipeline
                    ...
                    .apply("ParseDeltaJson", ParDo.of(new ParseStoreDeltaJsonDoFn()));
        } else {
    ...
    public static TableSchema buildSchema() {
        List<TableFieldSchema> fields = Arrays.asList(
                ...
                new TableFieldSchema().setName("adresse").setType("STRING"),
                new TableFieldSchema().setName("pays").setType("STRING"),
                new TableFieldSchema().setName("region").setType("STRING"),
                new TableFieldSchema().setName("surface").setType("INTEGER"),
                new TableFieldSchema().setName("date_ouverture").setType("STRING"),
                new TableFieldSchema().setName("statut").setType("STRING"),
                new TableFieldSchema().setName("nb_employes").setType("INTEGER"),
    ...
    public static class ParseStoreFullCsvDoFn extends DoFn<FileIO.ReadableFile, TableRow> {
        @ProcessElement
        public void processElement(@Element FileIO.ReadableFile file, OutputReceiver<TableRow> out) throws Exception {
            ...
            for (int i = 1; i < lines.length; i++) {
                ...
                TableRow row = new TableRow()
                        ...
                        .set("pays", cols[3])
                        .set("region", cols[4])
                        .set("surface", parseInteger(cols[5]))
                        .set("date_ouverture", cols[6])
                        .set("statut", cols[7])
                        .set("nb_employes", parseInteger(cols[8]))
                        .set("load_mode", "full")
    ...
    public static class ParseStoreDeltaJsonDoFn extends DoFn<FileIO.ReadableFile, TableRow> {
        ...
        public void processElement(@Element FileIO.ReadableFile file, OutputReceiver<TableRow> out) throws Exception {
            ...
            for (JsonNode node : root) {
                ...
                TableRow row = new TableRow()
                        ...
                        .set("adresse", nullableText(node, "adresse"))
                        .set("pays", nullableText(node, "pays"))
                        .set("region", nullableText(node, "region"))
                        .set("surface", nullableInt(node, "surface"))
                        .set("date_ouverture", nullableText(node, "date_ouverture"))
                        .set("statut", nullableText(node, "statut"))
                        .set("nb_employes", nullableInt(node, "nb_employes"))
                        .set("load_mode", "delta")
